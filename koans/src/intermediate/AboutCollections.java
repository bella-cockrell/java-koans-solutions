...
public class AboutCollections {
    ...
    public void usingBackedSubMap() {
        ...
        map.put("e", "Emu");
        map.put("f", "Fox");
        SortedMap<String, String> backedMap = map.subMap("c", "f"); // subMap returns a portion view of the original map
                                                                    // -- still linked
        assertEquals(backedMap.size(), 2); // from c to but not including f
        assertEquals(map.size(), 5);
        backedMap.put("d", "Dog");
        assertEquals(backedMap.size(), 3);
        assertEquals(map.size(), 6); // altering one map changes the other
    ...
    public void differenceBetweenOrderedAndSorted() {
        ...
        assertEquals(sorted.contains("z"), true);
