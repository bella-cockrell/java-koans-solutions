...
public class AboutAssertions {
    ...
    public void assertSameInstance() {
        Integer original = new Integer(1); // new Integer is depreciated, but just having original = 1 yields different results
        Integer same = original;
        Integer different = new Integer(1);
        // These are both equal to the original...
        assertEquals(original, same); // checks value?
        assertEquals(original, different); // checks value again?
        // ...but only one refers to the same instance as the original.
        assertSame(original, same); // checks if same instance?
    }
    ...
    public void assertNotSameInstance() {
        ...
        // original and same are the same
    }
}
